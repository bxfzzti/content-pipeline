...
## v0.8.0 — 2026-07-19: 热点筛选收敛为一次模型判断

**核心变更**：把“模型反复调度和自行评分”改为“代码预筛 + 一次语义判断 + 代码复算”，同时固定全网热点、关注方向、选题建议的输出顺序。

### 变更内容

- 热点展示固定为“全网热点 → 我关注的方向 → 选题建议”，全网热点按社会、国际、财经、体育、文娱、健康、科技等类别整理。
- 新增 `01c-screening-candidates.json`，由代码完成时效门槛、同事件去重和最多 12 条短名单预筛。
- 新增一次性语义筛选：模型只判断情绪强度、内容关联度、核心判断、推荐角度、反面理由和读者认知起点。
- 新增 `finalize_screening.py`，由代码统一复算 50 分制写作价值分、等级、排序、品牌上限和品类配比。
- 正常输出至少 5 条、最多 10 条选题；合格候选不足时说明缺口，不再用旧题或低质量题凑数。
- 新增 `validate_stage.py`，校验两层热点顺序、短名单数量、模型判断完整性和最终分数。
- 新增 `run_topic_scout.py`，将热点抓取和一次筛选串成一个入口。
- 保持个人小红书登录态禁用；产品体验线继续独立于新闻热点榜。

### 验证结果

- 新增确定性筛选与阶段校验测试，相关测试全部通过。
- 本机真实抓取 90 个来源，32.17 秒生成两层热点和 12 条短名单。
- 模拟完整判断后生成 10 条正式建议，热点与筛选阶段均通过契约校验。
