...
3. **必须保留确认点**：热点与选题确认、角度确认、定调与文章计划确认、发布确认。普通“跑流程”进入默认交互模式；只有“直接跑完”“全自动”“不用确认”等明确指令可跳过前三个确认点，质检不能跳过。
4. **禁止个人小红书登录态**：不得调用 xhs-cli、xiaohongshu-mcp、浏览器 Cookie、CDP 登录页或本地小红书 Cookie。只允许处理用户主动提供的链接、截图、导出内容，以及无需个人登录态的公开索引。
...
调用 `angle-selection`，必要时采集 KOL 近 48-72 小时观点。输出候选后进入**角度确认**，未经确认不得定调。
...
输出 `/tmp/article-pipeline/03-article-framework.md`，随后进入**定调与文章计划确认**，未经确认不得写正文。
