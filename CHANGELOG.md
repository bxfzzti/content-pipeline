...
## v0.7.0 — 2026-07-16: 两分钟全覆盖与交互确认恢复

**核心变更**：热点阶段改为有硬截止时间的并行编排，恢复选题、角度、定调三次确认，并永久禁止内容创作流借用个人小红书登录态。

### 变更内容

- 新增 `full_hotspot_run.py`，并行抓取 hot-aggregator、19 个国际/家居 RSS 和产品体验池。
- 每个来源输出 `live`、`cache`、`unavailable` 或 `disabled` 状态，实时失败时使用最近有效缓存。
- 热点阶段全局截止为 120 秒；实测 90 个来源状态在 14.8 秒内完成，得到 108 条关注领域热点和 81 条产品体验素材。
- 什么值得买 20 个关键词从串行改为最多 5 路并发，单关键词失败不影响其他关键词。
- 新增 `--sync-existing`，飞书 Base 同步复用本轮 JSON，不再二次抓取产品源。
- “跑流程”“跑一次流程”“一条龙”“写一篇文章”恢复为默认交互模式，强制停在选题、角度、定调三个确认点。
- 只有“直接跑完”“全自动”“不用确认”等明确表达才进入全自动模式。
- 禁止 `xhs-cli`、xiaohongshu-mcp、浏览器 Cookie、CDP 登录页和本地小红书 Cookie；保留的历史部署文档已标记停用。
- 热点评分不再把单篇小红书互动当作全网热度；缺少榜位或跨平台证据时只能标注为垂直讨论。
- 新增 12 个自动测试，覆盖截止时间、缓存降级、并发限制、结果复用、确认门和小红书安全契约。

### 实跑结果

- 总耗时：14.811 秒。
- 来源状态：90 个，其中实时 34、缓存 55、不可用 0、禁用 1。
- 登录态小红书运行标记：0。
- 飞书同步复用：80 条记录，产品源文件时间戳未变化。
